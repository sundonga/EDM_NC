...
int readCondition()
{
	...
	int k=0;
	fp=fopen("lock.txt","r");//以读模式打开lock.txt，如果打开失败，说明lock.txt文件不存在，则重新建立lock.txt文件。
	...
	if(fp==NULL)
	{
		fp=fopen("lock.txt","w");
		if(fp==NULL)return 1;
		for(c=0;c<conditionLength;c++)putc(int('0'),fp);
		fclose(fp);
	}
	fclose(fp);
	fp=fopen("condition.csv","r");
	if(fp==NULL)return 1;//读取condition.csv失败返回1
	fpp=fopen("temp.txt","w");//temp.txt为加工参数文件一副本，用于在程序运行时修改加工参数，在程序退出时将该文件内容复制至condition.csv文件中
	if(fpp==NULL)return 1;//读取temp.txt失败返回1
	fppp=fopen("tempa.txt","w");//tempa.txt为加工参数文件另一副本，用于在readCondition操作中协助读取文件
	if(fppp==NULL)return 1;//读取tampa.txt失败返回1
	fs=fopen("seek.txt","w");
	if(fs==NULL)return 1;
	while((c=getc(fp))!=int('\n'))  putc(c,fppp);//复制condition.csv第一行至tampa.txt
	c=int('\n');
	putc(c,fppp);
	fclose(fppp);
	rewind(fp);//将fp指针重置至文件头
	fppp=fopen("tempa.txt","r");//重新打开tampa.txt，以读格式
	if(fppp==NULL)return 1;//读取tempa.txt失败返回1
	startW(fpp,fs,conditionLength);//预设temp.txt和seek.txt文件内容
	rewind(fp);
	copyF(fp,fpp,fppp,fs);//将condition.csv文件内容复制至temp.txt中
	//关闭各文件
	fclose(fp);
	fclose(fpp);
	fclose(fppp);
...
void startW(FILE* fp,FILE* fs,int startL)
{
	int i,j;
	char a;
	a=' ';
...
void copyF(FILE* fp,FILE* fpp,FILE* fppp,FILE* fs)
{
	char c;
	int sign=0;
	while((c=(char)getc(fp))!=',');//读取第一行第一列的无用信息
	firstLine(fp,fpp,fs,sign);//读取第一行加工参数文件标题信息
	otherLine(fp,fpp,fppp,sign);//读取加工参数信息
}
void firstLine(FILE* fp,FILE* fpp,FILE* fs,int& sign)
{
	int now;
	int next;
	int pre;
	...
	while(sign!=1)
	{
		next=getFILE(fp,sign);//得到加工参数文件名序号即E****的****；
		//写入seek.txt用于索引
		...
		fseek(fs,long((now*12)*sizeof(char)),SEEK_SET);
		for(j=0;j<i;j++)fputc(int(temp[j]),fs);
		itoa(next,temp,10);
		//next、当前文件的后一个文件位置
		for(i=0;i<6;i++)
		{
		...
		fseek(fs,long((now*12+6)*sizeof(char)),SEEK_SET);
		for(j=0;j<i;j++)fputc(int(temp[j]),fs);
	...
	fseek(fs,long((0*12)*sizeof(char)),SEEK_SET);
	for(j=0;j<i;j++)fputc(int(temp[j]),fs);
	itoa(pre,temp,10);
	for(i=0;i<6;i++)
	{
...
//得到加工参数文件标题信息即E****中的****
int getFILE(FILE* fp,int&s)
{
	int i;//临时变量
	char temp[20];//临时变量
	(char)fgetc(fp);//获得首字母E，直接丢弃
	for(i=0;i<20;i++)
	{
...
void writeF(FILE* fp,int length)
{
	char temp[5];
	int i,j;
	//定位指针至该文件序列位置
	fseek(fp,long((length*6-1)*sizeof(char)),SEEK_SET);
	itoa(length,temp,10);
	for(i=0;i<5;i++)
	{
		if(temp[i]=='\0') break;
	...
	fputc(int(','),fp);
	fputc(int('E'),fp);
	for(j=0;j<i;j++)fputc(int(temp[j]),fp);
}
//本函数功能为将加工参数信息写入temp.txt文件中
void otherLine(FILE* fp,FILE* fpp,FILE* fppp,int&sign)
{
	int count;
	int i,j;//临时变量
	int temp;
	...
	{
		rewind(fppp);
		fseek(fpp,long((((conditionLength+1)*length+2)*count)*sizeof(char)),SEEK_SET);//定位temp.txt指针
		sign=0;

		//写入加工参数文件项目名称
		while((temp=fgetc(fp))!=int(','))
		{
			a=char(temp);
			fputc(int(temp),fpp);
		}
		...
		while(sign==0)
		{
			temp=getFILE(fppp,sign);//得到该参数所对应的文件名称
			if(sign==2)sign=0;
			...
			fseek(fpp,long((((conditionLength+1)*length+2)*count+temp*length-1)*sizeof(char)),SEEK_SET);//定位temp.txt指针
			fputc(int(','),fpp);
			for(j=0;j<i;j++)fputc(int(charT[j]),fpp);
		}
		count++;
	}
}
