...
int getCond(int line,int row)
{
	int i;//临时变量
	char temp[20];//临时变量
	fseek(fpTemp,long((((conditionLength+1)*length+2)*(row)+(line)*length-1)*sizeof(char)),SEEK_SET);//定位temp.txt文件指针至对应的位置
...
int fnSearch(HWND hWnd,int ENO)
{
	...
	while(1)
	{
		fseek(fpSeek,long((findID*12+6)*sizeof(char)),SEEK_SET);//从seek.txt的第一个文件位置开始搜索直至最后一个
		a[0]=(char)fgetc(fpSeek);
		if(a[0]==' ') 
		{
			return 0;
...
void fn_actionAdd(HWND hWnd)
{
	...
	if(ENO!=-1)
	{
		...
		else 
		{
			...
			while(i<12)
			{
				fseek(fpTemp,long((((conditionLength+1)*length+2)*i+ENO*length-1)*sizeof(char)),SEEK_SET);
				fputc(int(','),fpTemp);
				fputc(int('0'),fpTemp);
				i++;
			}
...
void fn_actionDele(HWND hWnd)
{
	...
	if(NOWSELECTLISTID!=0)
	{
		...
		for(i=0;i<11;i++)
		{
			fseek(fpLock,long(((ENO-1)*11+i)*sizeof(int)),SEEK_SET);
			fwrite(&j,sizeof(int),1,fpLock);
		}
		i=0;
		while(i<12)
		{
